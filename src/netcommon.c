...
/*
 * $Id: netcommon.c 1.5 02/06/24 14:53:19-00:00 twouters@ $ 
 */
...
void raw_notify_raw(player, msg, append)
dbref player;
const char *msg;
char *append;
{
...
int first;
{
	...
	*arg++ = '\0';
    ...
    if (d->flags & DS_CONNECTED && !strcmp(command, "hudinfo")) {
    	d->command_count++;
	...
	*--arg = ' ';		/*
	...
	return 1;
    } else if (((d->cstatus & C_REMOTE) || (d->cstatus & C_CCONTROL)) &&
	first) {
	if (!strncmp(command, "CONC ", sizeof("CONC ") - 1))
		...
		do_connectid(d, atoi(command + sizeof("Conn ID: ") - 1),
		    n + 1);
