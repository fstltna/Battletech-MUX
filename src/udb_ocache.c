 ...
 * 
 * $Id: udb_ocache.c 1.1 02/01/03 00:59:54-00:00 twouters@ $
 */
...
extern void logf();
extern int dddb_del(Objname * nam);
extern int dddb_put(Obj * obj, Objname * nam);
...
static Cache *get_free_entry(sp)
CacheLst *sp;
{
    Cache *cp;
	...
	if (!cache_busy && !cache_frozen && (cp = sp->mold.tail) != CNULL) {
	    /*
	     * unlink old cache chain tail 
	     */
	    if (cp->prv != CNULL) {
...
static Attr *get_attrib(anam, obj)
Aname *anam;
Obj *obj;
{
    int lo, mid, hi;
				 ...
				 */
{
    ...
    a = (Attrib *) realloc(obj->atrs,
	(obj->at_count + 1) * sizeof(Attrib));
...
Obj *obj;
{
	...
	anam->attrnum, obj->name, anam->object);
#endif
