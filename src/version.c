...
/*
 * $Id: version.c 1.1 02/01/03 01:00:23-00:00 twouters@ $ 
 */
...
#else
    sprintf(mudstate.version, "TinyMUX Beta version %s #%s", MUX_VERSION,
	MUX_BUILD_NUM);
#endif				/*
				 * PATCHLEVEL 
				 */
...
#else
    sprintf(mudstate.version, "TinyMUX version %s #%s [%s]", MUX_VERSION,
