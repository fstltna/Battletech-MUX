...
/*
 * $Id: econ_cmds.c 1.5 02/04/03 11:30:19-00:00 twouters@ $
 *
...
void SetCargoWeight(MECH * mech)
{
	...
	if (pile[i]) {
	    if (IsWeapon(i))
		sw = 10.24 * MechWeapons[Weapon2I(i)].weight;
	    else if (IsAmmo(i))
		sw = 1024;
	    else if (IsBomb(i))
		sw = 102 * BombWeight(Bomb2I(i));
#ifndef BT_PART_WEIGHTS
	    else if (IsSpecial(i) && i <= I2Special(CLAW))
		sw = 1024;
#else
	    else if (IsSpecial(i))	/* && i <= I2Special(LAMEQUIP) */
		sw = internalsweight[Special2I(i)];
	    else if (IsCargo(i))
		sw = cargoweight[Cargo2I(i)];
#endif				/* BT_PART_WEIGHTS */
	    else
...
#else
	    if (IsWeapon(id))
		sw = 10.24 * MechWeapons[Weapon2I(id)].weight;
	    else if (IsAmmo(id))
		sw = 1024;
	    else if (IsBomb(id))
		sw = 102 * BombWeight(Bomb2I(id));
	    else if (IsSpecial(id))
		sw = internalsweight[Special2I(id)];
	    else if (IsCargo(id))
		sw = cargoweight[Cargo2I(id)];
	    else
		sw = 102;
