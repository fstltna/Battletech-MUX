 ...
 * 
 * $Id: udb_ochunk.c 1.2 02/01/09 23:30:06-00:00 twouters@ $
 */
...
int dddb_init()
{
    ...
    for (block_size = 1; block_size < LBUF_SIZE;
	block_size = block_size << 1);

    if ((dbp =
	    gdbm_open(dbfile, block_size, GDBM_WRCREAT, 0600,
		gdbm_panic)) == (GDBM_FILE) 0) {
...
Objname *nam;
{
	...
	STARTLOG(LOG_ALWAYS, "BUG", "CRUPT") {
	    log_text((char *)
		tprintf("Database is corrupt, object %d. Exiting.",
		    (int) ret->name));
	    ENDLOG;
